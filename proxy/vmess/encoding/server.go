...
func (s *ServerSession) DecodeRequestHeader(reader io.Reader) (*protocol.RequestHeader, error) {
	...
	case errorAEAD == vmessaead.ErrNotFound:
		...
		if s.isAEADForced {
			return nil, drainConnection(newError("invalid user: VMessAEAD is enforced and a non VMessAEAD connection is received. You can still disable this security feature with environment variable v2ray_vmess_aead_forced = false . You will not be able to enable legacy header workaround in the future."))
		}
